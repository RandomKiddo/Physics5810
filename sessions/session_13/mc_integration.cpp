...
main (void)
{
  ...
  for (Nvec = Nvec_min; Nvec <= Nvec_max; Nvec *= 2)
  {
    ...
    for (int j = 0; j < Nvec; j++)
    {
       ...
       integral_avg += integrand2(rng_ptr,dim,sigma);  
    }
...
{ 
  ...
  for (int k = 1; k <= dim; k++)
  {
    // Generate x according to the Gaussian distribution N(0, sigma)
    double x_tmp = gsl_ran_gaussian(rng_ptr, sigma);
    x_sum += x_tmp;
  }

  // The w(x) weight is handled by the distribution itself,
  // so we only return the f(x) part.
  return (x_sum * x_sum); 
}