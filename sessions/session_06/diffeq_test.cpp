...
main (void)
{
  ...
  ofstream out ("diffeq_test4.dat");	// open the output file 
  ...
  out << "#      t           y_euler(t)         y_rk2(t)        y_exact(t) \n";
  ...
  for (double t = tmin; t <= tmax; t += h)
  {
    ...
    runge2 (N, t, y_rk4, h, rhs, params_ptr);	        // 4th order R-K 
  ...
  cout << "data stored in diffeq_test4.dat\n";
  out.close ();			// close the output file 

  ofstream out2 ("diffeq_test5.dat");
  out2 << "#      t           y_euler(t)         y_rk4(t)        y_exact(t) \n";
  out2 << scientific << setprecision (9)
    << tmin << "  "
    << y_euler_2[0] << "  "
    << y_rk4_2[0] << "  " << exact_answer (tmin, params_ptr) << endl;
  ...
  for (double t = tmin; t <= tmax; t += h)
  {
    ...
    runge2 (N, t, y_rk4_2, h, rhs, params_ptr);	        // 4th order R-K 

    out2 << scientific << setprecision (9)
      << t + h << "  "
      << y_euler_2[0] << "  "
      << y_rk4_2[0] << "  " << exact_answer (t + h, params_ptr) << endl;
  }

  cout << "data stored in diffeq_test5.dat\n";
