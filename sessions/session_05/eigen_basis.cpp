...
int main ()
{
    hij_parameters ho_parameters; 
    int potential_choice = 1; // 1 for Coulomb, 2 for Square Well
    double mass = 1.0;
    double b = 1.0;  // 1.0 for Coulomb, 0.5 for Square Well
    double E_exact = -0.5; // -0.5 for Coulomb, 45.9321 for Square Well
    
    ho_parameters.potential_index = potential_choice;
    ho_parameters.mass = mass;
    ho_parameters.b_ho = b;

    // Range for b: start, end, and step size

    ofstream fout("dim_search.dat");

    fout << "# N_parameter    Ground_State_Energy" << endl;

    for (int dimension = 2; dimension <= 45; ++dimension)
    {

        // Allocate GSL resources
        ...
        // Get the lowest eigenvalue (Ground State)
        double E0 = gsl_vector_get(Eigval_ptr, 0);

        // Output in "b E" format for plotting
        fout << fixed << setprecision(4) << dimension << "    " 
             << scientific << setprecision(10) << abs(E0-E_exact)/abs(E_exact) << endl;

        // Clean up memory inside the loop to avoid leaks
    ...
    cout << "Outputted to dim_search.dat" << endl;
    fout.close();
