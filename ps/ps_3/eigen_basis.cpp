...
//      03/06/26  added code for approximate wavefunction file output
//
//  Notes:
 ...
 * From our output PDF of the different basis sizes with a fixed b=1.0 (which
 * is the optimal value we found in Session 5). We see the behavior fits the 
 * behavior we saw in Session 5 of the relative error vs. basis size N. For small
 * basis sizes, there is a larger relative error, due to the way the basis functions
 * have to fit the value at r=0 and the long tail of the function. As we increase
 * the basis size, we get a much better approximation. As the basis nears convergence,
 * i.e., we keep increasing N, we see minimal returns. This means, the jump in accuracy
 * from N=1 to N=5 is much more noticeable than that of N=10 to N=20, and will continue
 * to be as such going forward. We have confirmed the wave functions are normalized
 * in the code itself.
 */
...
#include <cmath>
#include <sstream>    // for dynamic strings
...
main ()
{
  ...
  // Here is where we will add the wavefunction export behavior
  // We fetch the corresponding state eigenvector
  int state = 0; // the state we want, 0 = ground state
  gsl_vector *state_vec = gsl_vector_alloc(dimension);
  gsl_matrix_get_col(state_vec, Eigvec_ptr, state);

  // Sometimes GSL gets a negative vector (which is valid).
  // We flip it, we only care for positive.
  if (gsl_vector_get(state_vec, 0) < 0) {
    gsl_vector_scale(state_vec, -1.0);
  }

  // Open the file with dynamic file name and output a header
  stringstream ss;
  ss << "wf_state" << state << "_dim" << dimension << ".dat";
  ofstream fout(ss.str());
  fout << "# Wavefunction for Coulomb; State=" << state << ", Dim=" << dimension << "\n"
       << "# r      Approx.      Exact\n";
  
  // Radial steps to use
  double r_max = 6.0;
  double dr = 0.05;
  double sum = 0.0;  // to confirm normalization

  // Iterate over the radial grid
  for (double r = dr; r <= r_max; r += dr) {
    double psi_approx = 0.0;

    // Iterate over our dimension size and use eigenvalue coefficients to approximate psi
    for (int n = 0; n < dimension; ++n) {
      double coeff = gsl_vector_get(state_vec, n);
      psi_approx += coeff*ho_radial(n+1, 0, b_ho, r);
    }

    // The exact reduced psi value
    double psi_exact = 2.0*r*exp(-r);
    ...
    sum += (psi_approx * psi_approx) * dr;  // Crude normalization check |psi^2| dr
  }
  cout << "Normalization Integral: " << sum << "\n"; // Should be close to 1
  // Close the file and free the state vector
  fout.close();
  gsl_vector_free(state_vec);

  // Output where the file is saved (the filename)
  cout << "Wavefunction saved to " << ss.str() << "\n";
